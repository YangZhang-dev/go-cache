...
func TestCacheExpire(t *testing.T) {
	...
	table.Add(k+"_1", v+"_1", 250*time.Millisecond)
	table.Add(k+"_2", v+"_2", 200*time.Millisecond)
...
func TestCacheKeepAlive(t *testing.T) {
	...
	p := table.Add(k, v, 250*time.Millisecond)
...
func TestCallbackQueue(t *testing.T) {
	...
	table.RemoveDeleteItemCallback()
	secondItemKey := "itemKey02"
	expired = false
	i = table.Add(secondItemKey, v, 500*time.Millisecond)
	i.SetAboutToExpireCallback(func(key interface{}) {
		m.Lock()
		expired = true
		m.Unlock()
